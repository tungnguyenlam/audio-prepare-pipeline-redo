...
def main() -> int:
    ...
    p.add_argument('-im', '--input-manifest', type=Path, required=True, help='Path to diarization segments.json manifest')
    p.add_argument('-om', '--output-manifest', type=Path, help='Scored output manifest (default: dynamic per family under .data/speaker/score/<family>/segments.json)')
    p.add_argument('-p', '--profile', required=True, help='Enrolled speaker profile name or directory containing profile.json')
    p.add_argument('-pd', '--profiles-dir', type=Path, default=ROOT / '.data' / 'speaker_profiles', help='Profiles root directory')
    p.add_argument('-m', '--model-id', default=DEFAULT_EMBEDDING_MODEL_ID, help='Pyannote embedding model ID')
    ...
    p.add_argument('-w', '-ow', '--overwrite', action='store_true', help='Overwrite existing scored output manifest')
    p.add_argument('-c', '--concurrency', type=int, default=1, help='Number of concurrent workers for scoring turns. Set > 1 to enable concurrent execution')
    p.add_argument('-b', '-bs', '--batch-size', type=int, default=1, help='Number of turns to batch per scoring task')
    args = p.parse_args()
