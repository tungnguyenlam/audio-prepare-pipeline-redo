...
def main() -> int:
    ...
    p.add_argument('-im', '--input-manifest', type=Path, required=True, help='Manifest containing candidate turns')
    p.add_argument('-om', '--output-manifest', type=Path, help='Purity verification output manifest (default: dynamic per family under .data/speaker/purity/<family>/segments.json)')
    ...
    p.add_argument('-w', '-ow', '--overwrite', action='store_true', help='Overwrite existing purity output manifest')
    p.add_argument('-c', '--concurrency', type=int, default=1, help='Number of concurrent workers for verifying turns. Set > 1 to enable concurrent execution')
    p.add_argument('-b', '-bs', '--batch-size', type=int, default=1, help='Number of turns to batch per purity task')
    args = p.parse_args()
