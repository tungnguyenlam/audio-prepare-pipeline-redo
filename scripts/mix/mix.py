...
def main() -> int:
    ...
    p.add_argument('--sample-rate', type=positive_int, default=48000, help='Output audio sample rate in Hz')
    p.add_argument('--channels', type=int, choices=(1, 2), default=2, help='Output audio channel count (1=mono, 2=stereo)')
    p.add_argument('--peak-ceiling-dbfs', type=float, default=-1.0, help='Peak normalization ceiling in dBFS')
    p.add_argument('--output-dir', type=Path, default=ROOT / '.data/mix/out', help='Output directory for mixed and reference stems')
    p.add_argument('--work-dir', type=Path, default=ROOT / '.data/mix/work', help='Working directory for intermediate staging files')
    p.add_argument('--overwrite', action='store_true', help='Overwrite existing mixed stems and sidecars')
    p.add_argument('--concurrency', type=positive_int, default=1, help='Number of concurrent workers. Set > 1 to enable concurrent execution')
    p.add_argument('--batch-size', type=positive_int, default=1, help='Batch size for stem publishing')
    args = p.parse_args()
