...
def main() -> int:
    ...
    p.add_argument('--speech', type=Path, required=True, help='Speech source audio file path')
    p.add_argument('--music', type=Path, required=True, help='Music/background source audio file path')
    p.add_argument('--smr-db', type=float, required=True, help='Target Speech-to-Music Ratio in decibels')
    p.add_argument('--seed', type=int, required=True, help='Random seed for deterministic music section offset')
    p.add_argument('-sr', '--sample-rate', type=positive_int, default=48000, help='Output audio sample rate in Hz')
    p.add_argument('-ch', '--channels', type=int, choices=(1, 2), default=2, help='Output audio channel count (1=mono, 2=stereo)')
    p.add_argument('--peak-ceiling-dbfs', type=float, default=-1.0, help='Peak normalization ceiling in dBFS')
    p.add_argument('-od', '--output-dir', type=Path, default=ROOT / '.data/mix/out', help='Output directory for mixed and reference stems')
    p.add_argument('-wd', '--work-dir', type=Path, default=ROOT / '.data/mix/work', help='Working directory for intermediate staging files')
    p.add_argument('-w', '-ow', '--overwrite', action='store_true', help='Overwrite existing mixed stems and sidecars')
    p.add_argument('-c', '--concurrency', type=positive_int, default=1, help='Number of concurrent workers. Set > 1 to enable concurrent execution')
    p.add_argument('-b', '-bs', '--batch-size', type=positive_int, default=1, help='Batch size for stem publishing')
    args = p.parse_args()
