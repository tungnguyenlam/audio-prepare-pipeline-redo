...
def main() -> int:
    p = parser(__doc__, 's2-separate', 'mel_roformer')
    p.add_argument('-m', '--model', default='melband-roformer-kim-vocals',
    ...
    with contextlib.redirect_stdout(sys.stderr):
        session = MelBandRoformerSession(model_name=args.model, device=None if args.device == 'auto' else args.device)
        session.load()
