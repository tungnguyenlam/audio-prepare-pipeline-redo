"""Standalone bs_roformer inference with one session per invocation."""
...
def main() -> int:
    p = parser(__doc__, 's2-separate', 'bs_roformer')
    p.add_argument('-m', '--model', default='roformer-model-bs-roformer-sw-by-jarredou',
    ...
    args = p.parse_args()
    pairs = destinations(args, '_bs_roformer')
