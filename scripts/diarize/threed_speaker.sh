...
if [[ -z "$python_bin" ]]; then
    ...
    elif [[ -x "$repo_root/.venv-3dspeaker/bin/python" ]]; then
        python_bin="$repo_root/.venv-3dspeaker/bin/python"
    else
        python_bin="$repo_root/.venvs/3dspeaker/bin/python"
    fi
fi
...
exec "$python_bin" "$repo_root/scripts/diarize/threed_speaker.py" "$@"
