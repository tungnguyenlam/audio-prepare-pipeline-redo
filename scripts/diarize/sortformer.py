...
class _Sortformer:
    ...
    def diarize(
        ...
        *,
        ...
        if self._model is None:
            raise RuntimeError(
                "Sortformer checkpoint is not loaded. "
        ...
        with tempfile.TemporaryDirectory(prefix="sortformer-", dir=self.work_dir) as temp_dir_name:
    ...
    def _looks_like_native_segment(value: list[Any] | tuple[Any, ...]) -> bool:
        ...
        try:
            ...
            return False
        return _Sortformer._parse_speaker_index(speaker) is not None
    ...
    @staticmethod
    def _merge_adjacent_turns(turns: list[_Turn]) -> list[_Turn]:
        by_speaker: dict[str, list[_Turn]] = {}
        for turn in turns:
...
def main() -> int:
    ...
    keys = ('model_id', 'revision', 'model_filename', 'checkpoint_path', 'device', 'batch_size',
            ...
            'onset', 'offset', 'pad_onset_s', 'pad_offset_s', 'min_duration_on_s', 'min_duration_off_s')
    parameters = {key: getattr(args, key) for key in keys}
    args.work_dir.mkdir(parents=True, exist_ok=True)
    with contextlib.redirect_stdout(sys.stderr):
        model = _Sortformer(**parameters, work_dir=args.work_dir)
        model._load()
    ...
    def process(src, dest):
        ...
        wanted = request(identity(src), 'diarize', {**parameters, 'sample_rate': rate, 'channels': args.channels}, 'sortformer')
