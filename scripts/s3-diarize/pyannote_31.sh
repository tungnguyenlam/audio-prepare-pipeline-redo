...
if [[ ! -x "$python_bin" ]]; then
    echo "Missing model environment: $python_bin. Run: ./envs/setup_worker_envs.sh pyannote" >&2
    exit 2
fi
exec "$python_bin" "$repo_root/scripts/s3-diarize/pyannote.py" --model pyannote/speaker-diarization-3.1 "$@"
