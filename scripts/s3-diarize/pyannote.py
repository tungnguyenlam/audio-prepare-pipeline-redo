...
def main() -> int:
    p = parser(__doc__, 's3-diarize', 'pyannote_community1', segments=True)
    p.add_argument('-m', '--model', default='pyannote/speaker-diarization-community-1',
    ...
    def process(src, dest):
        ...
        wanted = request(identity(src), 'diarize', {**({'merge': merge_options} if args.merge else {}),
                         ...
                         **long_segment_parameters(args, vad_report)}, model_name)
