...
def main() -> int:
    p = parser(__doc__, 's3-diarize', 'diarizen', segments=True)
    p.add_argument('-m', '--model', default='BUT-FIT/diarizen-wavlm-large-s80-md-v2', help='HuggingFace model repository or local checkpoint directory')
    p.add_argument('-d', '--device', default='auto', help='Inference device ("auto", "cpu", "cuda", or "hip")')
    p.add_argument('-b', '-bs', '--batch-size', type=positive_int, default=1, help='Inference batch size for segmentation and embedding models')
    p.add_argument('-ns', '--num-speakers', type=positive_int, help='Exact number of speakers if known in advance')
    ...
    p.add_argument('-sr', '--sample-rate', type=positive_int, help='Output target sample rate in Hz (defaults to source sample rate)')
