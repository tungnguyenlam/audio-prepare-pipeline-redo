#!/usr/bin/env python3
"""Evaluate Gemma 4 E4B Q8_0 across all 31 Khanh Vy turns and update results."""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(".data/experiment_khanhvy/gemma_e4b_q8.log", mode="a", encoding="utf-8"),
    ],
)
logger = logging.getLogger("e4b_q8")
...
def query_e4b_q8(audio_path: Path, timeout_s: float = 120.0) -> dict:
    ...
    parsed = _normalize_result(content, backend="Unsloth-E4B-Q8")
    return {
        "raw_content": content,
        "reasoning": reasoning,
...
def main() -> None:
    ...
    logger.info("Starting Gemma 4 E4B Q8_0 evaluation across %d turns...", len(records))
    ...
    for idx, r in enumerate(records):
        ...
        logger.info("[%d/%d] Evaluating %s on Gemma 4 E4B Q8_0...", idx + 1, len(records), turn_id)

        try:
            res_q8 = query_e4b_q8(wav_path)
            logger.info("  -> E4B Q8_0: %s in %.2fs | Reason: %s", res_q8["decision"], res_q8["latency_s"], res_q8["reason"])
            r["gemma4_e4b_q8"] = res_q8
        except Exception as exc:
            logger.error("  -> E4B Q8_0 FAILED: %s", exc)
            r["gemma4_e4b_q8"] = {"error": str(exc), "decision": "error"}
    ...
    logger.info("=== EVALUATION OF GEMMA 4 E4B Q8_0 COMPLETE ===")
