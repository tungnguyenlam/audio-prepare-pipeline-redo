...
def main() -> int:
    ...
    p.add_argument("-o", "-of", "--output-file", type=Path, required=True, help="Path to write evaluation results JSON")
    p.add_argument("--title", type=str, default="Verifier Evaluation", help="Optional report title")
    p.add_argument("-w", "-ow", "--overwrite", action="store_true", help="Overwrite existing evaluation results JSON")
    p.add_argument("-c", "--concurrency", type=int, default=1, help="Number of concurrent workers for loading verdict files")
    p.add_argument("-b", "-bs", "--batch-size", type=int, default=1, help="Batch size of verdict files to read per worker task")
    args = p.parse_args()
