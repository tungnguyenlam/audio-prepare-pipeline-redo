...
| Parameter | Type & Range | Default | Mechanism | Increasing (+) Effect | Decreasing (-) Effect | Trade-off | Certainty |
|---|---|---|---|---|---|---|---|
| **`enable_gemma`** | `bool` `{True, False}` | `False` | Direct-audio speaker-purity and word-completeness verification via local Gemma or Gemini API. | Adds one request per candidate and rejects impurity, clipped boundaries, uncertainty, and errors. | Bypasses LLM evaluation, reducing latency/cost and retaining more candidates. | Acoustic Quality & Lower Yield vs. Inference Latency/Cost | **Empirical** LLM judgment. |
| **`gemma_backend`** | `str` `{"gemma4", "gemini"}` | `"gemma4"` | Selects local OpenAI-compatible Gemma or Google Gemini. | Gemini reads `GEMINI_API_KEY` only on the server and reports estimated paid-Standard cost. | Local GPU usage vs. metered API | **Guaranteed** routing; **estimated** cost. |
