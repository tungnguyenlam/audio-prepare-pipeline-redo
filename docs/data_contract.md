...
| [**`SpeakerProfile`**](07_data_contracts.md#4-speaker-profile-schema-speakerprofile) | `src/diarization/schemas.py` | Globally reusable known-speaker enrollment identity anchored by reference audio clips. |
...
| [**`OverlapVerificationResult`**](07_data_contracts.md#overlapverificationresult-multimodal-llm-purity) | `src/diarization/OverlapVerifier.py` | Multimodal direct-audio overlap classification result (`overlap: bool`, `reason: str`). |
...
1. **File-Backed State:** Memory waveforms are never persisted across public interfaces. File paths are repository-relative (e.g. `.data/separation/out/...`) to support cross-machine rsync portability.
