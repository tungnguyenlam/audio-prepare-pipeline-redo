...
| | `scripts/agent/verifier/gemini.sh` | `--input-file` / `--input-dir`, `--output-file` / `--output-dir`, `--model`, `--concurrency`, `--max-tokens`, `--temperature`, `--prompt-file` | Gemini direct-audio response `.txt` plus verdict JSON |
