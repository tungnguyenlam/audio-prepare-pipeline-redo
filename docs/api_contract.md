...
3. **Boundary & Syllable Integrity Gate (Premature Truncation Prevention):**
   ...
   - **Option C: Syllable & Word Forced Alignment Lock (`enable_syllable_alignment`):** Employs `whisper_timestamped` (default, supporting fine-tuned Vietnamese checkpoints such as `vinai/PhoWhisper-small`, `vinai/PhoWhisper-large`, `Viet-Speech/whisper-large-v3-vietnamese` or standard Whisper weights), PyTorch MMS-FA (`mms_fa`), or remote Whisper endpoints (`remote_whisper`). Snaps boundaries to word/syllable bounds, rescues chopped word codas into silence, and attaches recognized transcripts to turn records. Explicitly supports CPU inference (`aligner_device="cpu"`, recommended to prevent VRAM competition with diarizers) and features automatic CUDA OOM recovery (clears VRAM cache and transparently retries on CPU).
