...
echo "========================================================"
echo "  Setting up isolated Kimi-Audio environment (.venvs/kimi)"
echo "========================================================"

# Kimi-Audio runs best in an isolated environment with submodules cloned.
if [ "$1" = "--clean" ] || [ "$1" = "--recreate" ]; then
    echo "Cleaning existing .venvs/kimi..."
    rm -rf .venvs/kimi
fi

uv python install 3.11
uv venv .venvs/kimi --python 3.11

PY="$REPO_ROOT/.venvs/kimi/bin/python"
...
echo "Installing FlashAttention wheel with uv into .venvs/kimi (--no-build)..."
uv pip install --python "$PY" \
    --no-build \
    "$WHEEL_SRC"
...
echo "4/4. Installing Kimi-Audio package into .venvs/kimi (--no-deps)..."
uv pip install --python "$PY" \
    -e "$KIMI_DIR" \
    --no-deps
...
echo "🎉 .venvs/kimi is ready!"
echo ""
echo "To evaluate Kimi-Audio-7B-Instruct, run:"
echo ".venvs/kimi/bin/python scripts/evaluate_verifier.py \\"
echo "  --backend hf_local \\"
echo "  --model moonshotai/Kimi-Audio-7B-Instruct \\"
echo "  --output-json .data/tts_strategy/gold_benchmark_20260908/reports/kimi_audio_7b.json \\"
echo "  --output-report .data/tts_strategy/gold_benchmark_20260908/reports/kimi_audio_7b.md \\"
echo "  --export-csv .data/tts_strategy/gold_benchmark_20260908/reports/kimi_audio_7b.csv"
