...
echo "========================================================"
echo "  Setting up isolated MiniCPM-o environment (.venvs/minicpmo)"
echo "========================================================"

if [ "${1:-}" = "--clean" ] || [ "${1:-}" = "--recreate" ]; then
    echo "Cleaning existing .venvs/minicpmo..."
    rm -rf .venvs/minicpmo
fi

uv python install 3.11
uv venv .venvs/minicpmo --python 3.11

PY=".venvs/minicpmo/bin/python"
...
uv pip install --python "$PY" -r "$REPO_ROOT/envs/requirements-minicpmo.txt"
