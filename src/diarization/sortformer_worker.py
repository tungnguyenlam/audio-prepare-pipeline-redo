"""JSON-lines worker entrypoint for isolated NeMo Sortformer inference."""
...
def main() -> None:
    ...
    diarizer: SortformerDiarizer | None = None
    try:
            ...
            try:
                ...
                if action == "load":
                    if diarizer is not None:
                        raise RuntimeError("Sortformer worker is already loaded")
                    diarizer = SortformerDiarizer(**request["config"])
                    diarizer.load()
                    ...
                    raise ValueError(f"Unknown Sortformer worker action: {action!r}")
